...
- **Planning sub-agents must use `lattice plan` to find the plan path (2026-04-29):** When spawning a planner in a worktree with `LATTICE_ROOT` set, the agent's `Write` tool calls bypass `LATTICE_ROOT` — it writes to whatever absolute path it computed. Two planners in the mobile-polish wave (ASMV-87, ASMV-96) computed `${WORKTREE}/.lattice/plans/<uuid>.md` and wrote into the worktree's stale local `.lattice/` instead of the canonical primary one. The CLI then blocked the next `in_progress` transition with "plan is still scaffold" because the canonical plan file was empty. Fix: planner prompts must instruct the agent to call `lattice plan <task_id>` (the CLI honors `LATTICE_ROOT`) and Write to that exact returned path. Belt-and-braces: orchestrator should diff `worktree-*/.lattice/plans/` against `primary/.lattice/plans/` after each planner finishes and copy any stragglers.
- **`LATTICE_ROOT` points at the parent of `.lattice/`, not at `.lattice/` itself (2026-04-29):** The official `worktree-guide.md` example sets `LATTICE_ROOT=$(cd /path/to/.lattice && pwd)`, but the lattice CLI errors `NOT_INITIALIZED` on that — it expects the directory *containing* `.lattice/`. `scripts/new-lattice-worktree.sh` uses the working form. Worth flagging upstream to the Lattice maintainers.
